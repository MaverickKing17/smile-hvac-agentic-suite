...
import { XCircle, CheckCircle, Zap, Layers, ArrowRight } from 'lucide-react';
    ...
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto items-stretch">
          {/* Passive Design Card */}
          <div className="relative group flex flex-col bg-[#F9FAFB] border border-slate-200 rounded-[2rem] p-10 text-left transition-all duration-300">
            <div className="absolute top-6 right-8 bg-[#E5E7EB] text-slate-500 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">
              CURRENT STATE
            </div>
            
            <div className="mb-10">
              <h3 className="text-2xl font-black text-slate-700 mb-2">Passive Design</h3>
            </div>
            ...
            <ul className="space-y-10 flex-1">
              {[
                ...
                <li key={i} className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="font-black text-slate-700 text-sm uppercase tracking-tight">{item.title}</p>
          ...
          <div className="relative group flex flex-col h-full">
            <div className="absolute -inset-1 bg-emerald-500/10 rounded-[2.1rem] blur opacity-75 group-hover:opacity-100 transition duration-1000"></div>
            <div className="relative bg-white border-2 border-emerald-100 rounded-[2rem] p-10 text-left h-full flex flex-col shadow-2xl shadow-emerald-100/40">
              <div className="absolute top-6 right-8 bg-[#ECFDF5] text-[#10B981] px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 border border-emerald-100">
              ...
              <div className="mt-12 pt-8 border-t border-slate-50">
                <button onClick={() => window.dispatchEvent(new Event('smile-trigger-ai'))} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2 group">
                  Preview Solution <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </div>
          </div>
