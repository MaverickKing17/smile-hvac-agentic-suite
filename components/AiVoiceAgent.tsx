...
async function decodeAudioData(
  ...
  try {
    ...
    const alignedData = bufferLength % 2 !== 0 ? data.slice(0, bufferLength - 1) : data;
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            console.log("Connected to Smile AI");
            setStatus('listening');
            setIsActive(true);
        ...
        config: {
          ...
          systemInstruction: `
            ...
            - Friendly, patient, and ethical.
            - Expertise: 2026 Home Renovation Savings (HRS) program.
            - Logic: Mention rebates up to $7,500 (electric) or $2,000 (gas).
            - Collect: Name, Phone, and Heating Type.
            
            SECONDARY PERSONA: Sam (Emergency Dispatch)
            - Calm, fast, authoritative. 
            - Trigger: gas smell, no heat, leak, loud noise.
            - Hand-off Rule: Chloe says: "That sounds urgent. Let me get Sam, our emergency specialist, on the line for you."
            - Sam Rule: Ask for address, confirm 4-hour response guarantee.
            
            MANDATORY SAFETY RULE:
            If "gas smell" is mentioned, Sam must say: "For your safety, please hang up, leave the house immediately, and call 911. Once you are safe, call us back and we will dispatch a tech."
            
            Start as Chloe: "Thanks for calling Smile HVAC! Are you calling for a repair or about the $7,500 rebates?"
          `,
        }
      });
            ...
            {!isActive ? (
              <>
                ...
                <button 
                  ...
                  className="w-full bg-smileRed hover:bg-red-700 text-white font-black py-4 rounded-2xl shadow-xl shadow-red-500/20 transition-all active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest text-[10px] disabled:opacity-50"
                >
              ...
              <>
                <div className="relative flex items-center justify-center py-6 w-full">
                  <div className="flex gap-1.5 h-16 items-center">
                    {[...Array(12)].map((_, i) => (
                      <div key={i} className={`w-1.5 bg-smileRed rounded-full transition-all duration-150 ${status === 'speaking' ? 'animate-pulse' : 'opacity-40'}`} style={{ height: status === 'speaking' ? `${30 + Math.random() * 40}px` : '6px', animationDelay: `${i * 0.1}s` }} />
                    ))}
                  </div>
                </div>
